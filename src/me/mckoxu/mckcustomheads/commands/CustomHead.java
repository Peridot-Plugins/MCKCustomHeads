...
public class CustomHead implements CommandExecutor, Listener{
	...
	public static ItemStack customp = new ItemStack(Material.SKULL_ITEM, 1, (short) 3);{
		...
		for (String s : config.getStringList("mainpanel.items.customplayerskull.lore")) {
			lore.add(ChatColor.translateAlternateColorCodes('&', s));
		}
		im.setLore(lore);
		im.setDisplayName(ChatColor.translateAlternateColorCodes('&', config.getString("mainpanel.items.customplayerskull.name")));
	...
	public static ItemStack customs = new ItemStack(Material.SKULL_ITEM, 1, (short) 3);{
		...
		ArrayList<String> lore = new ArrayList<String>();
		for (String s : config.getStringList("mainpanel.items.customskull.lore")) {
			lore.add(ChatColor.translateAlternateColorCodes('&', s));
		}
		im.setLore(lore);
		im.setDisplayName(ChatColor.translateAlternateColorCodes('&', config.getString("mainpanel.items.customskull.name")));
	...
	public static ItemStack skullistitem = new ItemStack(Material.SKULL_ITEM, 1, (short) 3);{
		...
		for (String s : config.getStringList("mainpanel.items.skullist.lore")) {
			lore.add(ChatColor.translateAlternateColorCodes('&', s.replace("{AMOUNT}", String.valueOf(amount))));
		}
		im.setLore(lore);
		im.setDisplayName(ChatColor.translateAlternateColorCodes('&', config.getString("mainpanel.items.skullist.name")).replace("{AMOUNT}", String.valueOf(amount)));
		skullistitem.setItemMeta(im);
	}
	
	public static Inventory maininv;
	public CustomHead(){
		maininv = Bukkit.createInventory(null, 27, ChatColor.translateAlternateColorCodes('&', config.getString("mainpanel.name")));
	}
	@Override
		...
		if(cmd.getName().equalsIgnoreCase("customhead")){
			if(s instanceof Player){
				...
				if(p.hasPermission("mck.headsmenu")){
						...
						if(args[0].equalsIgnoreCase("give")){
							if(p.hasPermission("mck.headgive")){					
								if(args.length >= 2){
									...
									p.sendMessage(ChatColor.translateAlternateColorCodes('&', config.getString("messages.correctusage")));
									p.sendMessage(ChatColor.translateAlternateColorCodes('&', config.getString("messages.texture")));
								}
							}else{
								p.sendMessage(ChatColor.translateAlternateColorCodes('&', config.getString("error.noperm")));
							}
						} else{
							p.sendMessage(ChatColor.translateAlternateColorCodes('&', config.getString("messages.correctusage")));
							p.sendMessage(ChatColor.translateAlternateColorCodes('&', config.getString("messages.notrequired")));
						}
					} else{
						p.openInventory(inv);
						for (int i = 0; i < 27; i++) {
					...
					p.sendMessage(ChatColor.translateAlternateColorCodes('&', config.getString("messages.noperm")));
				}
			} else{
